...
def calculate_squeeze_score(
    ...
    # Squeeze flag: BB percentile < 10 AND ATR ratio < threshold
    is_squeeze = bool(percentile < 10 and atr_ratio < atr_threshold)

    # Count consecutive squeeze days from the end of the percentile array
    days_in_squeeze = 0
    atr_vals = atr_ratio_series.values
    min_len = min(len(pct_arr), len(atr_vals))
    for k in range(1, min_len + 1):
        p = pct_arr[-k]
        a = float(atr_vals[-k]) if not pd.isna(atr_vals[-k]) else 1.0
        if p < 10 and a < atr_threshold:
            days_in_squeeze += 1
        else:
            break
